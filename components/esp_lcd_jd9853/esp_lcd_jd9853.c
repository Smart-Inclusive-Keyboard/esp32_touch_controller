 ...
 *
 * JD9853 LCD panel driver for the Waveshare ESP32-C6-Touch-LCD-1.47.
 * Hardware parameters verified on that board:
 *   - 172x320 resolution, 34-pixel x-gap at rotation 0
 *   - colour inversion enabled
 *   - 80 MHz SPI clock
 */
                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    if (panel_dev_config->vendor_config) {
        jd9853->init_cmds =
            ((jd9853_vendor_config_t *)panel_dev_config->vendor_config)->init_cmds;
        jd9853->init_cmds_size =
            ((jd9853_vendor_config_t *)panel_dev_config->vendor_config)->init_cmds_size;
    }
...
static const jd9853_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0xC4, (uint8_t []){0x00, 0x00, 0xA0, 0x79, 0x0B, 0x0A, 0x16,
                        0x79, 0x0B, 0x0A, 0x16, 0x82}, 12, 0},
    {0xC8, (uint8_t []){0x3F,0x32,0x29,0x29,0x27,0x2B,0x27,0x28,
    ...
    {0x2A, (uint8_t []){0x00, 0x22, 0x00, 0xCD}, 4, 0},  /* x: 34..205 */
    {0x2B, (uint8_t []){0x00, 0x00, 0x01, 0x3F}, 4, 0},  /* y:  0..319 */
    {0xDE, (uint8_t []){0x02}, 1, 0},
    {0xE5, (uint8_t []){0x00, 0x02, 0x00}, 3, 0},
    {0xDE, (uint8_t []){0x00}, 1, 0},
...
static esp_err_t panel_jd9853_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(
                            io, LCD_CMD_MADCTL,
                            (uint8_t []){jd9853->madctl_val}, 1),
                        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(
                            io, LCD_CMD_COLMOD,
                            (uint8_t []){jd9853->colmod_val}, 1),
                        TAG, "send command failed");

    const jd9853_lcd_init_cmd_t *init_cmds =
        jd9853->init_cmds ? jd9853->init_cmds : vendor_specific_init_default;
    uint16_t init_cmds_size =
        jd9853->init_cmds ? jd9853->init_cmds_size
                           : (uint16_t)(sizeof(vendor_specific_init_default) /
                                        sizeof(jd9853_lcd_init_cmd_t));
    ...
    ESP_LOGD(TAG, "init sequence done");
    return ESP_OK;
}
...
static esp_err_t panel_jd9853_invert_color(esp_lcd_panel_t *panel, bool invert)
{
    jd9853_panel_t            *jd9853 = __containerof(panel, jd9853_panel_t, base);
    esp_lcd_panel_io_handle_t  io     = jd9853->io;
    int cmd = invert ? LCD_CMD_INVON : LCD_CMD_INVOFF;
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, cmd, NULL, 0),
                        TAG, "send command failed");
    return ESP_OK;
}

static esp_err_t panel_jd9853_mirror(esp_lcd_panel_t *panel,
...
static esp_err_t panel_jd9853_swap_xy(esp_lcd_panel_t *panel, bool swap)
{
    jd9853_panel_t            *jd9853 = __containerof(panel, jd9853_panel_t, base);
    esp_lcd_panel_io_handle_t  io     = jd9853->io;
    if (swap) {
